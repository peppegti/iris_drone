...
class GUI(Thread):
	def __init__(self, m,parent):
		...
		self.StopButton=Button(parent, text="STOP")
		self.StopButton['background']="green"
		self.StopButton['foreground']="black"
		self.StopButton['command']=self.StopButton_Click
		self.StopButton.pack({"side":"right","padx":0,"pady":0})
...
#		self.SnakeButton.configure(image=self.image_snake)
	...
	def StopButton_Click(self):
		#threading.Thread(target=self.StopButton_Click).start
		self.lucchetto.acquire()
		self.missioni.c.set_vel(0,0,0,0,0,0)
		messagebox.showerror("ERROR","MISSION STOPPED")
...
def main():
	...
	t = Thread(target = myapp.Stop_ButtonClick, args =())
	t.start()
